...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pragati'] = Office.objects.raw("SELECT * FROM office_office LEFT JOIN (SELECT * FROM reports_karyakram LEFT JOIN (SELECT * FROM reports_pragati GROUP BY karyakram_id ORDER BY id DESC LIMIT 0,1) AS reports_pragati ON reports_karyakram.id = reports_pragati.karyakram_id ORDER BY reports_pragati.datesubmited DESC LIMIT 0,1) AS karyakram ON office_office.id=karyakram.office_id")
